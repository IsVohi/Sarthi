...
export default function WeekCard({ weekData, weekIndex, currentWeek }: { weekData: any; weekIndex: number; currentWeek: number }) {
    const isCurrent = weekData.week === currentWeek;
    const [isExpanded, setIsExpanded] = useState(isCurrent);

    const { toggleTaskCompletion, completeWeek } = useSarthiStore();

    // Group tasks by day (since API returns a flat array of dailyTasks)
    const tasksByDay = weekData.dailyTasks.reduce((acc: any, task: any) => {
        if (!acc[task.day]) acc[task.day] = [];
        acc[task.day].push(task);
        return acc;
    }, {});

    const handleTaskClick = (e: React.MouseEvent, day: string, taskTitle: string) => {
        e.stopPropagation();
        toggleTaskCompletion(weekData.week, day, taskTitle);
    };

    const handleCompleteWeek = (e: React.MouseEvent) => {
        e.stopPropagation();
        completeWeek(weekData.week);
        confetti({
            particleCount: 100,
            spread: 70,
            origin: { y: 0.6 },
    ...
    const headerBorderClass = weekData.completed
        ? "border-emerald-500/50 bg-emerald-500/[0.02]"
        : isCurrent
            ? "border-indigo-500/50 bg-indigo-500/[0.02] shadow-[0_0_20px_rgba(99,102,241,0.05)]"
            : "border-[#1e1e2e] hover:border-slate-700";

    const completedTasksCount = weekData.dailyTasks.filter((t: any) => t.completed).length;
    const totalTasksCount = weekData.dailyTasks.length;
        ...
        <div className={`glass-card bg-[#13131f] border-2 rounded-2xl overflow-hidden transition-all duration-300 ${headerBorderClass}`}>
            ...
            >
                <div className="flex items-center gap-4 flex-1">
                    <div className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold ${weekData.completed
                    ...
                    <div className="flex-1">
                        <h3 className={`font-bold text-lg mb-1 flex items-center gap-2 ${weekData.completed ? "text-emerald-100" : "text-white"}`}>
                            {weekData.theme}
                            {weekData.completed && <CheckCircle2 className="w-5 h-5 text-emerald-500" />}
                        </h3>
                ...
                <div className="flex items-center gap-4 shrink-0 pl-4">
                    <div className="text-right hidden xs:block">
                        <div className="text-sm font-semibold text-white">{completedTasksCount}/{totalTasksCount} Tasks</div>
                        ...
                        <div className="p-6 pt-2 border-t border-[#1e1e2e]/50">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 space-y-6">
                                    {Object.entries(tasksByDay).map(([day, dayTasks]: any) => (
                                        <div key={day}>
                                            ...
                                            <div className="space-y-2">
                                                {dayTasks.map((task: any) => (
                                                    <div
                                                        key={task.task}
                                                        onClick={(e) => handleTaskClick(e, day, task.task)}
                                                        className={`flex items-start gap-4 p-3 rounded-xl border transition-all cursor-pointer group hover:bg-[#1a1a24] ${task.completed
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex-1">
                                                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1">
                                                                ...
                                                                <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded border border-indigo-500/20 bg-indigo-500/10 text-indigo-400`}>
                                                                    {task.type}
                                                                </span>
                                                            </div>
                                                            <div className="flex items-center gap-3 text-xs text-slate-500">
                                                                ...
                                                                {task.resource && (
                                                                    <a href={task.resource} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center gap-1 hover:text-indigo-400 transition-colors">
